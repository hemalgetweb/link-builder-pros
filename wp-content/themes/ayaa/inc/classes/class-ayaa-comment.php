				...
				<div id="div-comment-<?php comment_ID(); ?>" class="comment-list-item d-flex">
                    ...
                    echo "<div class='comment-list-content'>";
                        echo "<div class='comment-contenet'>";
                            echo '<div class="ayaa-fz-comment-top-head-778">';
                                ...
                                echo '<div class="comment-btn-wrapper">';
                                    ...
                                    $comment_timestamp = sprintf( __( '%1$s at %2$s', 'ayaa' ), get_comment_date( '', $comment ), get_comment_time() );
                                    printf(
                                        '<a href="%s"><time class="ayaa-comment-time" datetime="%s" title="%s">%s</time></a>',
                            ...
                            echo '<p class="comment-text">';
                                ...
                                if ( '0' === $comment->comment_approved ) {
                                    ?>
                                    <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'ayaa' ); ?></p>
                                    <?php
                                }
                            echo '</p>';
                            $comment_reply_link = get_comment_reply_link(
                                array_merge(
                                    ...
                                    array(
                                        ...
                                        'before'    => '<div class="ayaa-fz-comment-replay-btn-448">',
                                        'after'     => '</div>',
                                    )
                                )
                            );
                            $by_post_author = ayaa_is_comment_by_post_author( $comment );
